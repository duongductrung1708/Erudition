...
export default function FavoritesPage() {
    ...
    const fetchChatbots = async () => {
      try {
        ...
        if (user.isChatbotCreator) {
          // Attempt server-side filtering if API supports it
          response = await getAllChatbots(userToken, { is_active: true });
        } else {
          response = await getAllChatbotFromUser(userToken);
        }

        // Filter only active chatbots client-side as a fallback
        const activeAgents = response.filter((agent) => {
          if (typeof agent.is_active !== "boolean") {
            console.warn(
              `Invalid is_active for agent ${agent.id}:`,
              agent.is_active
            );
            return true; // Default to active if is_active is missing or invalid
          }
  ...
  const handleAgentDetailsChatHistory = (chatbotId) => {
    if (user.isChatbotCreator) {
      navigate(`/favorite-response/${chatbotId}`);
    } else {
      navigate(`/user/favorite-response/${chatbotId}`);
    }
  };
    ...
    <>
      ...
      >
        <Box
          ...
          mb={"1rem"}
        >
          <Box fontWeight="bold" sx={{ display: "flex", alignItems: "center" }}>
            <IconButton
              sx={{
                color: "black",
                borderRadius: "10px",
              ...
              onClick={() =>
                navigate(
                  user.isChatbotCreator ? "/workspace" : "/user/workspace"
                )
              }
            >
              <ArrowBackIosIcon />
