...
const FavoritesPageForUser = () => {
  ...
  const fetchChatbotDetails = async () => {
    try {
      ...
      const matchedChatbot = chatbots.find(
      ...
      if (!matchedChatbot.is_active) {
        throw new Error("Chatbot is inactive and cannot be accessed");
      }

      setChatbotName(matchedChatbot.name || "Unnamed Chatbot");
    } catch (error) {
  ...
  useEffect(() => {
    ...
    fetchChatbotDetails();
    fetchFavorites();
  }, [chatbotId, user.accessToken, navigate]);
        ...
        >
          <Tooltip title="Back">
            <IconButton
              ...
              onClick={() => navigate("/user/favorite-response")}
            >
              <ArrowBackIosIcon />
            </IconButton>
          </Tooltip>
...
export default FavoritesPageForUser;
