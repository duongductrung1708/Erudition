...
import pandas as pd
# SQLModel removed - using MongoDB only
...
from .utils import (
    ...
    logger, tokens_need_and_delta_tokens,
)
from .types import KnowledgeGraph
from dotenv import load_dotenv

from ...api.routes.websocket import manager
from ...models_mongo import Document as DocumentModel, DocumentStatus
from ...services.DocumentServicesMongo import DocumentServicesMongo
from ...services.ChatbotServicesMongo import ChatbotServicesMongo
from ...utils_package.DataUtils import get_markdown_text_of_doc, \
    get_qa_data_text_of_doc, process_if_not_enough_token
...
class LightRAG:
    ...
    def __post_init__(self):
        from app.LightRAG.lightrag.kg.shared_storage import (
    ...
    async def apipeline_enqueue_documents(
        ...
        if not new_docs:
            ...
            await manager.send_status(db_obj.chatbot_id, "error", f"Document with same content is already upload!")
            return
        ...
        async with pipeline_status_lock:
            ...
            if not pipeline_status.get("busy", False):
                ...
                if not to_process_docs:
                    ...
                    await manager.send_status(db_obj.chatbot_id, "error", f"No documents to process!")
                    return
                ...
                await manager.send_status(db_obj.chatbot_id, "warn", f"Another process is already processing the document queue. Request queued.")
                return
        ...
        try:
            ...
            while True:
                ...
                async def process_document(
                    ...
                    try:
                        await DocumentServicesMongo.update_document_status(document_id=status_doc.db_obj_id, status=DocumentStatus.PROCESSING)
                    except:
                        logger.warning("Document status is not changed to PROCESSING yet.")
                    try:
                            ...
                            self.doc_status.upsert(
                                {
                                    doc_id: {
                                        ...
                                        "updated_at": datetime.now().isoformat(),
                                        "file_path": file_path,
                                        "db_obj_id": status_doc.db_obj_id
                                    }
                                }
                            )
                        )
                        ...
                        if db_obj.has_qa_data:
                            ct = await get_qa_data_text_of_doc(db_obj.chatbot_id, db_obj.id)
                        else:
                        ...
                        await manager.send_status(db_obj.chatbot_id, "success", f"{document_title} upload successfully")
                        # Update status to READY and add usage tokens - MongoDB version
                        udoc = await DocumentServicesMongo.get_document_by_id(document_id=status_doc.db_obj_id)
                        if udoc:
                            await DocumentServicesMongo.update_document_status(document_id=status_doc.db_obj_id, status=DocumentStatus.READY)
                            # Add usage tokens to chatbot
                            await ChatbotServicesMongo.add_usage_tokens(chatbot_id=udoc.chatbot_id, tokens=need, action="UPLOAD DOCUMENT")
                            # Add usage tokens to document
                            await DocumentServicesMongo.update_document(
                                document_id=status_doc.db_obj_id,
                                update_data={"usage_tokens": udoc.usage_tokens + need, "latest_modified": datetime.now()}
                            )
                        ...
                        # Update status to FAILED - MongoDB version
                        u = await DocumentServicesMongo.get_document_by_id(document_id=status_doc.db_obj_id)
                        if u:
                            await DocumentServicesMongo.update_document_status(document_id=status_doc.db_obj_id, status=DocumentStatus.FAILED)
                            await manager.send_status(chatbot_id=u.chatbot_id, status="error", detail=f"{u.document_title} processes failed")
    ...
    async def aquery(
        ...
        if param.mode in ["local", "global", "hybrid"]:
            ...
            logger.warning(f"Chưa sửa để lấy source trong mode {param.mode}")
            ...
            logger.warning(f"Chưa sửa để lấy source trong mode {param.mode}")
            query_result = await mix_kg_vector_query(
                query.strip(),
                self.chunk_entity_relation_graph,
                self.entities_vdb,
                self.relationships_vdb,
                ...
                if data_with_chunk:
                    ...
                    for item in data_with_chunk:
                        ...
                        if not new_sources:
                            ...
                            await vdb.delete_entity(item.get('id', ''))
                        else:
