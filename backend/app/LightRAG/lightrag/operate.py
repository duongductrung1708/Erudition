...
class QueryResult(str):  # Kế thừa từ str để giữ nguyên cách hoạt động
    def __new__(cls, response, extra_value):
        obj = super().__new__(cls, response)  # Tạo một str object
        obj.extra_value = extra_value  # Gán thêm giá trị mới
        return obj
...
def get_query_result(resp, src = None, len_tokens = 0):
    return {
        "response": resp,
        "source": src,
        "usage_tokens": len_tokens
    }
async def kg_query(
    query: str,
    knowledge_graph_inst: BaseGraphStorage,
    ...
    if cached_response is not None:
        return get_query_result(cached_response)

    # Extract keywords using extract_keywords_only function which already supports conversation history
    ke_result = await extract_keywords_only(
        query, query_param, global_config, hashing_kv
    )
    hl_keywords = ke_result["hl_keywords"]
    ll_keywords = ke_result["ll_keywords"]
    ...
    if hl_keywords == [] and ll_keywords == []:
        ...
        return get_query_result(PROMPTS["fail_response"])
    ...
    source = context_result["sources"]
    if query_param.only_need_context:
        return get_query_result(context, len_tokens=ke_result["usage_tokens"], src=source)
    if context is None:
        return get_query_result(PROMPTS["fail_response"])
    ...
    sys_prompt = sys_prompt_temp.format(
        ...
        chatbot_name=query_param.chatbot_model.name,
        chatbot_organization=query_param.chatbot_model.organization,
        chatbot_description=query_param.chatbot_model.description,
        guard_rails_text=query_param.guardrails
    )

    if query_param.only_need_prompt:
        return get_query_result(sys_prompt, len_tokens=ke_result["usage_tokens"], src=source)

    len_of_prompts = len(encode_string_by_tiktoken(query + sys_prompt)) + ke_result["usage_tokens"]
    ...
    return get_query_result(response, len_tokens=len_of_prompts, src=source)
...
async def mix_kg_vector_query(
    ...
    if query_param.only_need_context:
        return get_query_result({"kg_context": kg_context, "vector_context": vector_context}.__str__())
    ...
    if query_param.only_need_prompt:
        return get_query_result(sys_prompt)
    ...
    return get_query_result(response)
...
async def _build_query_context(
    ...
    if query_param.mode == "local":
        ...
        context_result = await _get_node_data(
            ll_keywords,
            knowledge_graph_inst,
            entities_vdb,
            text_chunks_db,
        ...
        logger.debug("Done got node data & edge data in local mode...")
    elif query_param.mode == "global":
        logger.debug("Global mode...")
        context_result = await _get_edge_data(
            hl_keywords,
            knowledge_graph_inst,
            relationships_vdb,
            text_chunks_db,
        ...
        logger.debug("Done got node data & edge data in global mode...")
    else:  # hybrid mode
        logger.debug("Hybrid mode...")
        ll_d, hl_d = await asyncio.gather(
            _get_node_data(
                ll_keywords,
                knowledge_graph_inst,
                entities_vdb,
        ...
        logger.debug("Done got node data & edge data in hybrid mode...")
        ll_data, hl_data = ll_d["contexts"], hl_d["contexts"]
        ll_srcs, hl_srcs = ll_d["sources"], hl_d["sources"]
        (
            ll_entities_context,
        ...
        logger.debug("Combining contexts...")
        context_result = combine_contexts(
            [hl_entities_context, ll_entities_context],
            [hl_relations_context, ll_relations_context],
            [hl_text_units_context, ll_text_units_context],
        )
        logger.debug("Combined contexts to context_result...")

        logger.debug("Start getting source data from context...")
        if ll_srcs is not None:
            for doc_id, idx_list in ll_srcs.items():
                context_result["sources"][doc_id].update(idx_list)
        if hl_srcs is not None:
            for doc_id, idx_list in hl_srcs.items():
                context_result["sources"][doc_id].update(idx_list)
        context_result["sources"] = {
            doc_id: sorted(list(idxs))
            for doc_id, idxs in context_result["sources"].items()
        }
        logger.debug("Done getting source data from context...")

    logger.debug("Fetching entities_context, relations_context, text_units_context from context_result.....")
    entities_context, relations_context, text_units_context = context_result["contexts"]
...
def get_context_result(ctxs = ("", "", ""), srcs = None):
    return {
        "contexts": ctxs,
        "sources": srcs
    }
async def _get_node_data(
    ...
    for i, t in enumerate(use_text_units):
        ...
        idx = t.get("chunk_order_index")
        doc_id = t.get("full_doc_id")
        if idx not in src[doc_id]:
            src[doc_id].append(idx)

    text_units_context = list_of_list_to_csv(text_units_section_list)
    contexts = entities_context, relations_context, text_units_context
    return get_context_result(contexts, src)
...
def combine_contexts(entities, relationships, sources):
    ...
    contexts = combined_entities, combined_relationships, combined_sources
    return get_context_result(contexts, defaultdict(set))
...
async def naive_query(
    ...
    if query_param.only_need_context:
        return get_query_result(section)
