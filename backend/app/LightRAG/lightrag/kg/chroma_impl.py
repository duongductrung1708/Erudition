...
from app.LightRAG.lightrag.base import BaseVectorStorage
from app.LightRAG.lightrag.utils import logger
...
class ChromaVectorDBStorage(BaseVectorStorage):
    ...
    async def client_storage(self):
        """Returns all stored data in the collection."""
        try:
            # Lấy tất cả dữ liệu từ collection
            results = self._collection.get(include=["metadatas", "documents", "embeddings"])
            # Nếu collection trống, trả về danh sách rỗng
            ...
            logger.error(f"Error retrieving client storage data: {e}")
            return {"data": []}
